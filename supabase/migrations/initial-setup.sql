...
BEGIN
    ...
    -- Check if the policy for email_leads exists
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'email_leads' 
        AND policyname = 'Public can insert email leads'
    ) THEN
        -- Create policy for email leads - public insert only, no read
        EXECUTE 'CREATE POLICY "Public can insert email leads" ON public.email_leads
                FOR INSERT WITH CHECK (true)';
    END IF;

    -- Check if the policy for ideas exists
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'ideas' 
        AND policyname = 'Users can manage own ideas'
    ) THEN
        -- Create policy for ideas - users can only access their own ideas
        EXECUTE 'CREATE POLICY "Users can manage own ideas" ON public.ideas
